...
class CUPTRule(RuleInterface):
    ...
    def get_optional_question_id_list(
        ...
        if len(question_id_lib_list) > self.question_count:
            ...
            console.info(f"[bold]当前比赛可用题库为：[/bold][yellow]{temp_question_id_lib_list}[/yellow]")
            rep_qrecord_set = set([
            ...
            console.warning(f"[red]赛题小于 [yellow]{self.question_count}[/yellow] 道，无法进行赛题的禁用与解放[/red]")
    ...
    def get_rep_score_weight(self, team_record_data_list: List[RecordData], is_refuse: bool) -> float:
        ...
        Returns:
            float: 正方计分权重
        """
        console.info(f"[bold]getRepScoreWeight(teamRecordDataList: List<RecordData>, isRefuse: Boolean)[/bold]")
        console.info(f"[bold]teamRecordDataList = [/bold][yellow]{team_record_data_list}[/yellow]")
        console.info(f"[bold]isRefuse = [/bold][yellow]{is_refuse}[/yellow]")
        old_rep_score_weight_list = [it.weight for it in team_record_data_list if it.role in ("R", "X")]
        if len(old_rep_score_weight_list) == 0:
            console.info("[bold]return [/bold][yellow]3.0[/yellow]")
            return 3.0
        else:
            ...
            console.info(f"[bold]return [/bold][yellow]{weight}[/yellow]")
            return weight
